...
def getNumberOfDays(ticker):
    ...
    numOfDays = sparkSession.sql("SELECT NumberOfDays FROM optionData WHERE Ticker Like '" + ticker + "'").collect()
    numDaysArr = []
...
def getOptionPricesByTicker(ticker):
    ...
    return sparkSession.sql("SELECT Prices FROM optionData WHERE Ticker Like '" + ticker + "'").collect()
...
def getCallStrikePricesByTicker(ticker):
    ...
    prices = sparkSession.sql("SELECT Prices FROM optionData WHERE Ticker Like '"+ ticker +"'").collect()
    strikePricesArray = []
    for i in prices[0]['Prices']:
        if i['Call'] != None and i['StrikePrice'] not in strikePricesArray:
...
def getPutStrikePricesByTicker(ticker):
    ...
    strikePricesArray = []
    for i in prices[0]['Prices']:
        if i['Put'] != None and i['StrikePrice'] not in strikePricesArray:
            strikePricesArray.append(i['StrikePrice'])
    strikePricesArray.sort()
    return strikePricesArray
